...
pub async fn create_backend(backend_type: crate::GpuBackendType) -> Result<Box<dyn GpuBackend>> {
    match backend_type {
        ...
        crate::GpuBackendType::Cuda => Err(anyhow::anyhow!("CUDA backend not available in this build")),
...
pub struct CudaBackend {
    device: Device,
    #[cfg(feature = "cuda")]
    context: Option<cudarc::driver::CudaDevice>,
}

async fn create_cuda_backend() -> Result<Box<dyn GpuBackend>> {
    #[cfg(feature = "cuda")]
    {
        if cudarc::driver::CudaDevice::count()? > 0 {
            let device = Device::new_cuda(0)?;
            let context = Some(cudarc::driver::CudaDevice::new(0)?);
            let mut backend = CudaBackend { device, context };
...
impl GpuBackend for CudaBackend {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        #[cfg(feature = "cuda")]
        {
            // Initialize CUDA context and allocate memory pools
            if let Some(ref ctx) = self.context {
                info!("CUDA device: {:?}", ctx.ordinal());
            }
        }
        Ok(())
    }
    ...
    async fn extract_visual_features(&self, frames: &[VideoFrame]) -> Result<VisualFeatures> {
        ...
        for (idx, frame) in frames.iter().enumerate() {
            ...
            let tensor = frame_to_tensor(frame, &self.device)?;
...
async fn create_rocm_backend() -> Result<Box<dyn GpuBackend>> {
    ...
    #[cfg(feature = "rocm")]
    {
        ...
        anyhow::bail!("ROCm backend not yet implemented")
    }
...
async fn create_metal_backend() -> Result<Box<dyn GpuBackend>> {
    #[cfg(target_os = "macos")]
    {
        ...
        let mut backend = MetalBackend { device };
        backend.initialize().await?;
        return Ok(Box::new(backend));
    }
...
async fn create_openvino_backend() -> Result<Box<dyn GpuBackend>> {
    #[cfg(feature = "openvino")]
    {
        ...
        anyhow::bail!("OpenVINO backend not yet implemented")
    }
...
async fn create_cpu_backend() -> Result<Box<dyn GpuBackend>> {
    ...
    let mut backend = CpuBackend { device };
    backend.initialize().await?;
    Ok(Box::new(backend))
}
...
// Helper functions
fn frame_to_tensor(frame: &VideoFrame, device: &Device) -> Result<Tensor> {
    let shape = [1, 3, frame.height as usize, frame.width as usize];
    Tensor::from_vec(frame.data.clone(), shape, device)
        .context("Failed to create tensor from frame")
}
